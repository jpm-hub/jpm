...
func Bundle() {
	...
	if err == nil {
		for _, entry := range entries {
			os.RemoveAll(filepath.Join(dist, entry.Name()))
		}
	}
	...
	for i, arg := range os.Args {
		...
		case "-d":
			if len(os.Args) > i+1 {
				dist = filepath.Clean(os.Args[i+1])
				handled = append(handled, os.Args[i+1])
			} else {
...
func copyFromOut() {
	...
	if COM.IsWindows() {
		...
		os.RemoveAll(filepath.Clean(dist) + "\\_dump\\tests")
	} else {
